...
- Fetch: Contains the Program Counter (PC) Logic, that indexes the Instruction memory. If the C extension is enabled, it handles the proper alignment of instructions and "decompresses" them if needed.
- Decode and Operand Fetch: Decodes the instruction, extracting its type, format, operation and register addresses, also Fetches the operands in the register Bank and performs data hazard detections. When a hazard is detected, it inserts NOP instructions (bubbles) until the conflict is resolved.
- Execute: Performs the given operation on the received operands. Holds an Arithmetic and Logic Unit, a CSR access unit, a Memory Load and Store unit and a Branch unit. If the M extension is enabled, it includes the multiplier and divider units. It also includes the atomic unit for the A extension.
- Retire: Performs the write-back of the results of the instruction, it can be either the result from execute stage or the data read from memory. In the case of a LR.W atomic instruction, it saves the reservation set data.
