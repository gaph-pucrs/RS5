...
rvfi_monitor_context* rvfi_monitor_init(char* monitor_prefix, rv_isa isa, int en_tracer, int en_profiler, int en_checker, char* tracer_log_file_name, char* profiler_log_file_name, char* profiler_call_graph_file_name, char* symbol_table_file_name, char* symbol_watchlist_file_name) {
    ...
    if (ctx->en_tracer || ctx->en_profiler) {
        ...
        while (1) {
            ...
            // 'T' = global function, 't' == local function
            // if (symbol_type != 'T')
...
void rvfi_monitor_push_counters_to_stack(rvfi_monitor_context *ctx, symbol_info_t *current_symbol, const rvfi_trace_t *rvfi_trace) {
    ...
    current_counter_state->ret_pc = rvfi_trace->rvfi_pc_rdata + inst_length(rvfi_trace->rvfi_insn);  // NOTE: inst_length() comes from the MJC disassembly library, this function must be brought to this source file if migrating to libopcodes
...
void rvfi_monitor_step(rvfi_monitor_context *ctx, const rvfi_trace_t *rvfi_trace, uint64_t current_clock_cycle) {  // TODO: Pass simulation time as argument, print messages with timestamp
    ...
    if (ctx->en_profiler) {
        ...
        uint32_t control_flow_change = (rvfi_trace->rvfi_pc_wdata != (rvfi_trace->rvfi_pc_rdata + inst_length(rvfi_trace->rvfi_insn)));
