...
int test_shake_tv()
{
	...
	// shake_xof(&sha3);
	shake128_absorb(&sha3, in, rvkat_gethex(in, sizeof(in), "7216A825029DA1"));

	// shake_out(md, 16, &sha3);
	shake128_squeeze(&sha3, md, 16);
	...
	// shake_xof(&sha3);
	shake256_absorb(&sha3, in, rvkat_gethex(in, sizeof(in), "FDBFF9BF3FB910A92D25651EBBFBA365"));

	// shake_out(md, 32, &sha3);
	shake128_squeeze(&sha3, md, 32);
